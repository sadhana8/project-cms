...
{
$aColumns = array( 's.id','s.name','s.balance','s.fees','b.semester_name','s.phone','s.doj');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "s.id";
	
	/* DB table to use */
	$sTable = " student as s,semesters as b ";
	...
	{
	$condArr[] = "s.name like '%".mysqli_real_escape_string($conn,$_GET['student'])."%'";
	
	}
	
	if(isset($_GET['semesters']) && $_GET['semesters']!="")
	{
	$condArr[] = "b.id = '".mysqli_real_escape_string($conn,$_GET['semester_name'])."'";
	...
	$sWhere = " WHERE b.semester_id=s.semesters and s.delete_status='0' and s.balance>0 ";
	...
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE b.semester_id=s.semesters and s.delete_status='0' and s.balance>0  ";
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		...
		$row = array(
                    html_entity_decode($aRow['name'].'<br/>'.$aRow['phone']),
                    $aRow['fees'],
					$aRow['balance'],
                    $aRow['semesters'],
...
$aColumns = array( 's.id','s.name','s.balance','s.fees','b.semester_name','s.phone','s.doj');
	...
	if(isset($_GET['semesters']) && $_GET['semesters']!="")
	{
	$condArr[] = "b.semester_id = '".mysqli_real_escape_string($conn,$_GET['semester_name'])."'";
	...
	$sWhere = " WHERE b.semester_id=s.semesters and s.delete_status='0'  ";
	if ( isset($_GET['sSearch'])&& $_GET['sSearch'] != "" )
	{
	...
	/* Total data set length */
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE b.semester_id=s.semesters and s.delete_status='0'   ";
	$rResultTotal = $conn->query( $sQuery ) or die(mysqli_error($conn));
	$aResultTotal = $rResultTotal->fetch_assoc();
	$iTotal = $aResultTotal['cc'];
