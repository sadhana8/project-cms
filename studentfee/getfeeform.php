...
$sid = (isset($_POST['student']))?mysqli_real_escape_string($conn,$_POST['student']):'';
 ...
 $sql = "select s.id,s.name,s.balance,s.fees,s.phone,b.semesters,s.doj from student as s,semesters as b where b.id=s.semesters and  s.delete_status='0' and s.id='".$sid."'";
$q = $conn->query($sql);
if($q->num_rows>0)
{
...
echo '  <form class="form-horizontal" id ="signupForm1" action="fees.php" method="post">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" disabled  value="'.$res['phone'].'" />
    </div>
  </div>
...
if($fq->num_rows>0)
{
 ...
 $sql = "select s.id,s.name,s.balance,s.fees,s.phone,b.semesters,s.doj from student as s,semesters as b where b.id=s.semesters  and s.id='".$sid."'";
$sq = $conn->query($sql);
$sr = $sq->fetch_assoc();
